...
pub fn send_ping(addr: &IpAddr, timeout: Duration, data: &[u8], options: Option<&PingOptions>) -> Result<PingReply> {
    match addr {
        IpAddr::V4(_) => ping_sync::<Ipv4Addr>(addr, timeout, data, options),
        IpAddr::V6(_) => ping_sync::<Ipv6Addr>(addr, timeout, data, options),
    }
}

fn ping_sync<P: Proto>(addr: &IpAddr, timeout: Duration, data: &[u8], options: Option<&PingOptions>) -> Result<PingReply> {
    ...
    let mut context = PingContext { ident: process_id, sequence: 0, destination, payload, socket };
    context.ping()?;
    context.wait_reply::<P>()
}

pub async fn send_ping_async(addr: &IpAddr, timeout: Duration, data: Arc<&[u8]>, _options: Option<&PingOptions>) -> PingApiOutput {
    let timeout = validate_timeout(timeout)?;
    let payload = make_data::<Ipv4Addr>(&data)?;

    let socket = create_socket::<Ipv4Addr>()?;
...
impl PingContext {
    ...
    fn wait_reply<P: Proto>(&mut self) -> Result<PingReply> {
...
fn create_socket<P: Proto>() -> Result<Socket> {
    ...
    Socket::new_raw(domain, Type::DGRAM, Some(protocol))
        .map_err(|e| PingError::OsError(e.raw_os_error().unwrap_or(-1) as u32, format!("Create socket failed: {:?}", e.kind())))
}
